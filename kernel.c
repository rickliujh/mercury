...
struct sbiret
sbi_call(
    long arg0, long arg1, long arg2, long arg3, long arg4, long arg5, long fid,
    long eid
) {
...
alloc_pages(uint32_t n) {
    // static variable with initialization, this part is executed only once—when
    // the program starts (or when the variable is first defined in some
    // systems). After that, next_paddr keeps its value from the last function
    // call because of its static nature
